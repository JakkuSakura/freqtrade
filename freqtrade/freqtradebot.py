...
from freqtrade.persistence import HistoryRepository, Order, PairLocks, Trade, init_db
...
STOPLOSS_RETRY_KEY = "stoploss_retry_not_before"
...
class FreqtradeBot(LoggingMixin):
    ...
    def __init__(self, config: Config) -> None:
        ...
        # Init objects
        self.config = config
        self.state_events: StateEvents = StateEvents()
        self.state_store: StateStore = StateStore(self.state_events)
        self.state_reader: StateReader = StateReader(self.state_store)
        self.state_sync: DataSyncService = DataSyncService(self.state_store)
        self.oms: OrderManagementService = OrderManagementService(self.state_store)
        ...
        if self.trading_mode == TradingMode.FUTURES:
            ...
            def futures_job() -> None:
                self.update_funding_fees()
                self.update_all_liquidation_prices()
                self.wallets.update()

        self._scheduler.setup_core_jobs(
            trading_mode=self.trading_mode,
            refresh_positions_job=self.refresh_positions_orders_trades,
            reset_ws_job=self.exchange.ws_connection_reset,
            futures_maintenance_job=futures_job,
        )
    ...
    def startup_update_open_orders(self):
        ...
        for order in orders:
            ...
            # Get retry count for this order from custom data
            retry_count = 0
            retry_data = order.trade.get_custom_data_entry('exchange_error_retries')
            if retry_data:
                retry_data_values = retry_data.value if isinstance(retry_data.value, dict) else {}
                retry_count = retry_data_values.get(order.order_id, 0)

            # If we've exceeded retry threshold, assume order is gone and cancel it
            MAX_RETRIES = 3
            if retry_count >= MAX_RETRIES:
                logger.warning(
                    f"Order {order.order_id} has failed to update {retry_count} times. "
                    "Assuming it was filled or cancelled on the exchange."
                )
                fo = order.to_ccxt_object()
                fo["status"] = "canceled"
                self.handle_cancel_order(
                    fo, order, order.trade, constants.CANCEL_REASON["CANCELLED_ON_EXCHANGE"]
                )
                continue
            ...
            try:
                ...
                # Successfully fetched order - reset retry counter
                if retry_data:
                    retry_data_values = retry_data.value if isinstance(retry_data.value, dict) else {}
                    if order.order_id in retry_data_values:
                        retry_data_values.pop(order.order_id)  # Clear retry count on success
                        order.trade.set_custom_data('exchange_error_retries', retry_data_values)

                # Update trade state with fetched order
            ...
            except RetryableOrderError as e:
                # Handle orders no longer on exchange (filled, cancelled or expired)
                logger.warning(f"Order {order.order_id} not found on exchange: {e}")

                # For orders not found on exchange, mark them as cancelled
                fo = order.to_ccxt_object()
                fo["status"] = "canceled"
                self.handle_cancel_order(
                    fo, order, order.trade, constants.CANCEL_REASON["CANCELLED_ON_EXCHANGE"]
                ...
                # If it's a DDosProtection error during startup, we'll assume the order
                # might still be valid, but we're being rate limited. Skip it for now.
                logger.warning(
                    f"Rate limit hit while retrieving order {order.order_id}: {e}. "
                    "Skipping for now, will retry on next run."
                )
                # Don't increment retry counter for rate limits
                continue

            except (InvalidOrderException, ExchangeError) as e:
                # For any exchange error, increment the retry counter
                logger.warning(f"Error updating Order {order.order_id} due to {e}")

                # Update the retry counter
                retry_count += 1
                retry_data_values = {} if not retry_data else \
                    (retry_data.value if isinstance(retry_data.value, dict) else {})
                retry_data_values[order.order_id] = retry_count
                order.trade.set_custom_data('exchange_error_retries', retry_data_values)

                # If this was the last retry, mark the order as cancelled
                if retry_count >= MAX_RETRIES:
                    logger.warning(
                        f"Order {order.order_id} has reached maximum retry count ({MAX_RETRIES}). "
                        "Assuming order was filled or cancelled on the exchange."
                    )
                    fo = order.to_ccxt_object()
                    fo["status"] = "canceled"
                    self.handle_cancel_order(
    ...
    def execute_entry(
        ...
        # order already mirrored via OMS
    ...
    def cancel_stoploss_on_exchange(self, trade: Trade) -> Trade:
        ...
        for oslo in trade.open_sl_orders:
            try:
                ...
                if self.oms:
                    co = self.oms.cancel_order(
                        exchange=self.exchange,
                        trade=trade,
                        order=oslo,
                        amount=trade.amount,
                        reason=constants.CANCEL_REASON["USER_CANCEL"],
                        is_stoploss=True,
                    )
                else:
    ...
    def create_stoploss_order(self, trade: Trade, stop_price: float) -> bool:
        ...
        """
        retry_interval = self.strategy.order_types.get(
            "stoploss_on_exchange_retry_interval", 0
        )

        def schedule_retry(reason: str) -> None:
            if not retry_interval:
                return
            retry_not_before = datetime.now(UTC) + timedelta(seconds=retry_interval)
            trade.set_custom_data(STOPLOSS_RETRY_KEY, retry_not_before.timestamp())
            logger.info(
                "Will retry stoploss order for %s after %ss (not before %s). Reason: %s",
                trade.pair,
                retry_interval,
                retry_not_before,
                reason,
        ...
        if retry_interval:
            retry_not_before_ts = trade.get_custom_data(STOPLOSS_RETRY_KEY)
            if retry_not_before_ts:
                retry_not_before = datetime.fromtimestamp(float(retry_not_before_ts), tz=UTC)
                if datetime.now(UTC) < retry_not_before:
                    logger.debug(
                        "Skipping stoploss order for %s due to retry cooldown until %s.",
                        trade.pair,
                        retry_not_before,
                    )
                    return False
        try:
            if self.oms:
                order_obj, stoploss_order = self.oms.submit_stoploss(
                    exchange=self.exchange,
                    trade=trade,
                    amount=trade.amount,
                ...
                trade.orders.append(order_obj)
            if retry_interval:
                trade.set_custom_data(STOPLOSS_RETRY_KEY, None)
            return True
            ...
            logger.warning(
                "Temporary error while placing stoploss order for %s: %s", trade.pair, e
            )
        ...
        if order["status"] not in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            if replacing:
                ...
                while (
                    ...
                    if self.oms:
                        self.oms.apply_order_update(trade, order_obj, corder)
                    retry_count += 1
    ...
    def handle_cancel_exit(
        ...
        if order["status"] not in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            trade.exit_reason = None
            if self.oms:
                self.oms.apply_order_update(trade, order_obj, order)
    ...
    def update_trade_state(
        ...
        if not stoploss_order:
            logger.info(f"Found open order for {trade}: {order_id}")
        try:
    ...
    def _update_trade_after_fill(self, trade: Trade, order: Order, send_msg: bool) -> Trade:
        if order.status in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            if self.oms:
                self.oms.sync_trade(trade)
                self.oms.record_order_event(trade, order_obj, event="closed")
                if not trade.is_open:
                    self.oms.archive_trade(trade)
            else:
                self._sync_trade_state(trade)
                HistoryRepository.record_order_event(trade, order_obj, event="closed")
                if not trade.is_open:
    ...
    def order_close_notify(self, trade: Trade, order: Order, stoploss_order: bool, send_msg: bool):
        ...
        # Only send notifications for non-open orders
        if order.status in constants.NON_OPEN_EXCHANGE_STATES:
            if order.ft_order_side == trade.exit_side:
                # Exit notification
                if send_msg and not stoploss_order:
                    self._notify_exit(
                        trade, order.order_type, fill=True, sub_trade=trade.is_open, order=order
                    )
                if not trade.is_open:
                    self.handle_protections(trade.pair, trade.trade_direction)
            elif order.ft_order_side == trade.entry_side and send_msg and not stoploss_order:
                # Calculate if this is a sub-trade (DCA) based on order amount vs trade amount
                sub_trade = not isclose(
                    order.safe_amount_after_fee, trade.amount, abs_tol=constants.MATH_CLOSE_PREC
                )
                # Enter fill notification
